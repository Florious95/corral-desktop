...
| 终端列 Cmd/Ctrl-V 粘贴文本或图片上传 | **做**（2026-08-22 重做）：DOM paste；图片走 Rust HTTP | CORS 根因；不重开 loopback CSP |
...
13. **2026-08-22**：回退 #33 粘贴 v2 及其 CSP loopback HTTP 口子（用户接受文字粘贴一并退）。同日重做：文字粘贴回来；图片改走 Rust `POST`，**不**恢复 loopback `connect-src`。
