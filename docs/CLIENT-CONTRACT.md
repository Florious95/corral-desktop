...
隐藏序列；该序列不上传服务端，其他 provider 维持原有 xterm 游标行为。为保证中文 IME 不跟随停靠游标，
该模式还扫描可视缓冲区中的 `→/-> Add a follow-up` 行，并把 xterm 的
`.xterm-helper-textarea` 与 `.composition-view` 定位到该软件输入行；并监听 xterm 对两者 `style` 的内部冲刷，在 composition/input 生命周期及 DOM mutation 后重新锚定。仅修改本地 DOM，绝不改写远端帧或游标状态。
