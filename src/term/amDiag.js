 ...
 *
 * Restored from ca1f54c (t.evidence). ⛔ does not import garbleDetect.
 */

/** 10 rounds × ~50 sessions × ~20 events, with headroom. */
export const AM_DIAG_CAPACITY = 16384;

/** Quiet window after a label/write with no term_resize / write_snapshot. */
...
function touchSettle(ev) {
  ...
  if (ev.type === 'write_snapshot' || ev.type === 'snapshot_gate') {
    s.t_label = t;
    s.t_stable = null;
    s.quietGen += 1;
...
/** Counts only. Never returns decoded text. */
export function byteStats(u8) {
  const n = u8 && u8.length != null ? u8.length : 0;
  let esc = 0;
  let lf = 0;
  let cr = 0;
  let csi = 0;
  if (u8) {
    for (let i = 0; i < n; i++) {
      ...
      if (b === 0x1b) {
        esc += 1;
        if (i + 1 < n && u8[i + 1] === 0x5b) csi += 1;
      } else if (b === 0x0a) lf += 1;
      else if (b === 0x0d) cr += 1;
    }
  }
  return { bytes: n, esc, lf, cr, csi };
}
