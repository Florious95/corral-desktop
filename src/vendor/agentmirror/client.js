...
import {
  push as diag, recordHostGeom, forgetHostGeom, hostGeomOf,
  markSubscribed, recordLiveHostGeom, stampHostAtSnap, byteStats,
} from '../../term/amDiag.js';
...
export class Client {
  ...
  subscribe(ref, rows, cols) {
    ...
    const forceHost = typeof globalThis !== 'undefined' && globalThis.__amSubscribeAtHost === true;
    let sendRows = rows;
    let sendCols = cols;
    if (forceHost && host && host.rows >= 1 && host.cols >= 1) {
      sendRows = host.rows;
      sendCols = host.cols;
    }
    this.activeSubscriptions.set(ref, { rows: sendRows, cols: sendCols });
    const hostFields = {
      ...
      req_rows: rows,
      req_cols: cols,
      force_host: forceHost === true,
    };
    if (!this.isReady) {
      diag({ type: 'subscribe', ref, rows: sendRows, cols: sendCols, sent: false, reason: 'not_ready_bookkept', ...hostFields });
      return true; // bookkept; replayed on READY
    }
    const sent = this.sendControl('subscribe', { ref, rows: sendRows, cols: sendCols });
    if (sent) markSubscribed(ref);
    diag({ type: 'subscribe', ref, rows: sendRows, cols: sendCols, sent, reason: sent ? null : 'send_failed', ...hostFields });
    return sent;
  }
  ...
  handleMessage(data) {
    if (typeof data === 'string') {
      ...
      const stats = byteStats(frame.data);
      diag({
  ...
  handleFrame(type, payload) {
    switch (type) {
      ...
      case 'listing': {
        ...
        diag({ type: 'listing', ref: null, listing_seq: payload.seq, pane_count: panes.length, panes });
