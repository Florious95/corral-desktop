...
🔴 **回炉方法论（通用法则：代码未生效即腐败代码）**：
- **未生效即腐败**：依据需求改了代码但交付面未生效，就是腐败代码；不仅无用，还会留下副作用污染主树，严禁在腐败代码上继续叠补丁。
- **逐 PR 审计精确尝试**：两轮以上未根治的问题强制回炉；严禁看两眼静态代码猜想，必须由开发席深入 Git 逐 PR 审计：改了哪几行精确代码、尝试了什么、为何未生效，标定腐败代码整条回退。
