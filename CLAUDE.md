...
- 🔴 **回炉方法论（代码未生效即腐败代码，涉及高级顾问仅用户明确指令启动才启用）**：
  - **未生效即腐败**：依据需求改了代码但交付面未生效，就是腐败代码；不仅无用还会留下副作用污染主树，严禁在腐败代码上继续叠补丁。
  - **逐 PR 审计精确尝试**：两轮以上未根治的问题（且用户指令启动回炉），严禁看两眼静态代码猜想，必须由开发席深入 Git 逐 PR 审计：改了哪几行精确代码、尝试了什么、为何未生效，标定腐败代码整条回退。
  - **完备上下文门禁**：未经开发席输出 PR 审计证据链，顾问坚决拒绝盲审；定案后一事一 PR 纯净重写，并在真实交付面上验收（严禁 mock 报假绿）。
