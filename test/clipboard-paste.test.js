...
test('CSP connect-src has no loopback HTTP; upload is not webview fetch', async () => {
  ...
  for (const forbidden of [
    ...
    assert.equal(connect.includes(forbidden), false, `must not pin ${forbidden}`);
  }
  assert.equal(/\bhttps?:($|\s)/.test(connect), false, 'must not allow any-origin http:/https:');
  assert.match(connect, /\bipc:/);
...
test('chooseUploadTransport: native is not gated on fetchImpl === fetch', () => {
  assert.equal(chooseUploadTransport({ desktop: true }), 'native');
  assert.equal(chooseUploadTransport({ desktop: true, fetchImpl: fetch }), 'fetch');
  const poster = async () => {};
...
test('DeviceManager.uploadImage native postUpload path; pairing secret not in log', async () => {
  ...
  const id = dm.addDevice({ name: 'L', url: 'ws://127.0.0.1:9/ws', token: 'secret-token-value' });
  const uid = `${id}::s1`;
  dm._clients.set(id, { url: 'ws://127.0.0.1:9/ws', inputAttachment: () => 7 });
  const png = new File([new Uint8Array([1, 2, 3])], 'a.png', { type: 'image/png' });
  let calls = 0;
  const sent = await dm.uploadImage(uid, png, {
    postUpload: async (args) => {
      calls += 1;
      assert.match(args.url, /\/upload$/);
      assert.ok(String(args.authorization).startsWith('Bearer '));
      assert.equal(args.filename, 'a.png');
      assert.equal(args.mime, 'image/png');
      assert.equal(args.data.length, 3);
      return {
        ok: true, status: 200, body: '{"path":"/host/uploads/a.png"}',
        unreachable: false, err_name: '', err_message: '',
      };
    },
  });
  assert.equal(calls, 1);
  assert.equal(sent.reqId, 7);
  const recs = readUploadLogRing();
  assert.equal(recs.length, 1);
  assert.equal(recs[0].ok, true);
  assert.equal(JSON.stringify(recs).includes('secret-token-value'), false);
});

test('upload log records swallowed fetch error without pairing secret', async () => {
  clearUploadLogRing();
  const storage = {
    _m: new Map(),
    getItem(k) { return this._m.has(k) ? this._m.get(k) : null; },
  ...
  err.name = 'TypeError';
  try {
    await dm.uploadImage(`${id}::s1`, png, async () => { throw err; });
    assert.fail('expected throw');
  } catch (e) {
    assert.equal(e.message, '上传失败：无法连接');
  }
  ...
  assert.equal(recs[0].name, 'TypeError');
  assert.equal(recs[0].message, 'Load failed');
  assert.match(recs[0].url, /^http:\/\/127\.0\.0\.1:9\/upload$/);
  assert.equal(recs[0].unreachable, true);
  const blob = JSON.stringify(recs);
  assert.equal(blob.includes('secret-token-value'), false);
  assert.equal(/Bearer\s+\S+/i.test(blob), false);
  assert.equal(scrubLogText('Bearer secret-token-value x'), 'Bearer *** x');
  assert.equal(scrubLogText('secret-token-value'), '***');
});
