...
test('TabBar component matches UI-SPEC §4.1.1 and advisor requirements', async () => {
  ...
  // Status indicator lamps (working / idle / unknown - 2026-09-16 矫正)
  ...
  assert.match(chromeCss, /\.tb-tab-lamp\.is-working\s*\{[^}]*box-shadow:\s*0 0 6px rgba\(34, 197, 94, 0\.6\);/);
  assert.match(chromeCss, /\.tb-tab-lamp\.is-working\s*\{[^}]*animation:\s*tb-lamp-pulse/);
  assert.match(chromeCss, /\.tb-tab-lamp\.is-idle\s*\{[^}]*background:\s*rgba\(255,\s*255,\s*255,\s*0\.28\);/);
